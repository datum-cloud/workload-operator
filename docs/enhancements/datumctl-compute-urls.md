...
```
$ datumctl compute deploy api --image=ghcr.io/acme/api:1.4.2 --city=DFW,IAD --min=2 --http-port=8080
...
And in `describe`, with per-city backend health — the view that makes multi-city routing visible, which it previously was not:
...
datumctl compute workloads describe   URL, backend port, and per-city backend health
